...
if(K==0)
    ...
    doa_angle = [-30 45];
    doa_range = [9700 10000];
    range = [1.5e3 1.5e3 1.5e3];
end
...
% % mesh(zeros(size(theta1)) -xl(1),theta2,Pmax_db');
% % mesh(theta1, zeros(size(theta2)) -yl(1),Pmax_db');
% plot3(zeros(size(dis1)) + xl(1),dis2_paper,piquancy_origin','Color','#D95319');
% plot3(dis1_paper, zeros(size(dis2)) + yl(1),piquancy_origin','Color','#0072BD');
% xlim([9500 10500]);
% ylim([9500 10500]);
% hold off
% figure()
...
% % mesh(zeros(size(theta1)) -xl(1),theta2,Pmax_db');
% % mesh(theta1, zeros(size(theta2)) -yl(1),Pmax_db');
% plot3(zeros(size(dis1)) + xl(1),dis2,piquancy_one','Color','#D95319');
% plot3(dis1, zeros(size(dis2)) + yl(1),piquancy_one','Color','#0072BD');
% xlim([9500 10500]);
% ylim([9500 10500]);
% hold off
...
%%%%%%%%%%%%%%%%%%%%%%%%%paperplot one dimensional%
% figure;
% piquancy_one = zeros(1,length(Pmax_db));
% dim = length(Pmax_db);
% index_dim = 1;
% % for dim_i = 1:dim
% %     piquancy_one(1,index_dim) = max(Pmax_db, [], 1);
% %     index_dim = index_dim + 1;
% % end
% piquancy_one = max(Pmax_db, [], 1);
% plot(theta1,piquancy_one);
% xlabel('theta/(meter)');
% ylabel('piquancy func/(dB)');
%出很大的问题，但是一直没有找到原因和解决方法
%%%%%%%%%%%%%%%%%%%%%%%%%paperplot one dimensional%

% % %%%%%%%%%%%%%%%%% SIMULATION OF GSP APPROACH END %%%%%%%%%%%%%%%%%%

% % % %%%%%%%%%%%%%%%%% SIMULATION OF MUSIC APPROACH BEGIN %%%%%%%%%%%%%%%%%%
% figure();
% plot(dis1_full,Pmax_db_full,'lineWidth',3)
% hold on
% %load Pmax_db_full_r3nofull.mat;
% plot(dis1_full,Pmax_db_full_r3nofull,'lineWidth',3)
% hold on
% plot(dis1_full,piquancy_origin_trick_db,'lineWidth',3)
% legend('Full connecntion','None-Full connecntion','Target');
% set(gca,'FontName','Times New Roman','FontSize',30,'LineWidth',1);
% xlabel('distance/(meter)','FontSize',40);
% ylabel('piquancy func/(dB)','FontSize',40);
% ylim([-90,0])
% toc
% % % %%%%%%%%%%%%%%%%% SIMULATION OF MUSIC APPROACH END %%%%%%%%%%%%%%%%%%